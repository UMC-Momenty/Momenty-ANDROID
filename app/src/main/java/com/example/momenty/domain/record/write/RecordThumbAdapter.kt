...
class RecordThumbAdapter(
    private val onAddClick: () -> Unit,
    private val onRemoveClick: (Uri) -> Unit
) : RecyclerView.Adapter<RecordThumbAdapter.VH>() {
    ...
    override fun getItemCount(): Int =
        if (photos.size < MAX_COUNT) photos.size + 1 else photos.size
    ...
    inner class VH(
        ...
        fun bind(position: Int) {
            ...
            Glide.with(binding.ivThumb).load(uri).centerCrop().into(binding.ivThumb)
